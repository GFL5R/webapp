<template>
  <ContentFrame>
    ...
    <p>Passions are the things your character lives for — hobbies, obsessions, callings, and guilty pleasures. Each passion grants two benefits: a narrative truth about your character, and a mechanical way to remove strife by engaging with it.</p>
    ...
    <!-- Results count -->
    <p class="db-count">{{ filtered.length }} of {{ passions.length }} passions</p>
    ...
    <div v-else class="db-empty">
      <p>No passions match your filters.</p>
    </div>
...
const allTypes = [...new Set(passions.flatMap(p => p.types))].sort()
...
onMounted(() => {
  ...
  if (hash && passions.some(p => p.id === hash)) {
    expanded.value = hash
    // Scroll to element
    setTimeout(() => {
