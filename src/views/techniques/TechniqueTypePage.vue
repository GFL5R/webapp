<template>
  <ContentFrame>
    ...
    <!-- Results count -->
    <p class="db-count">{{ filtered.length }} of {{ typeTechniques.length }} techniques</p>
    ...
    <PageNav :prev="info.prev" :next="info.next" />
  </ContentFrame>
</template>
...
import { TECHNIQUE_TYPES } from '@/data/technique-types.js'

const route = useRoute()
const router = useRouter()

// ---- Derive info from route param ----
const info = computed(() => {
  const type = route.params.type
  return TECHNIQUE_TYPES[type] || TECHNIQUE_TYPES.combat
})

const crumbs = computed(() => [
  { label: 'Home', to: '/' },
  { label: info.value.title }
])
...
const typeTechniques = computed(() => {
  const entries = rawTechniques[info.value.dataKey] || []
  const result = []
...
const approachFilters = computed(() => {
  const set = new Set()
  typeTechniques.value.forEach(t => {
...
function hasTechnique(id) {
  return typeTechniques.value.some(t => t.id === id)
}

// ---- Filtered + sorted ----
const filtered = computed(() => {
  let results = typeTechniques.value
...
onMounted(() => {
  ...
  if (hash && typeTechniques.value.some(t => t.id === hash)) {
    expanded.value = hash
    setTimeout(() => {
